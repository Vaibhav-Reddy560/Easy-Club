            ...
            <header className="border-b border-white/5 pb-8 flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div className="space-y-4">
                    <div>
                        ...
                        <p className="text-white text-sm mt-1 uppercase font-bold tracking-widest">Universal Organization & Profile Intelligence</p>
                    </div>
                ...
                <div className="flex gap-4">
                    <button 
                        ...
                        className="px-5 py-3 bg-zinc-900/40 border border-white/5 rounded-2xl flex items-center gap-3 hover:bg-zinc-900 transition-all group"
                    >
                ...
                {activeClub && stats ? (
                    ...
                    >
                        ...
                        {Object.values(activeClub.socialConnections || {}).every(conn => !conn.isConnected) ? (
                            <div className="bg-zinc-900/40 border border-gold-500/10 rounded-[2.5rem] p-12 text-center space-y-8 relative overflow-hidden group">
                            ...
                            <div className="space-y-8">
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-4">
                                     ...
                                     <button 
                                         onClick={handleConnectSocial}
                                         className="px-4 py-2 border border-white/5 bg-black/40 rounded-xl text-[8px] font-black uppercase tracking-widest text-white hover:text-white transition-all underline decoration-gold-500/30"
                                     >
                                         Manage Protocols
                                     </button>
                                 </div>
                        ...
                        <div className="space-y-8 pt-8 border-t border-white/5">
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-4">
                                    <div className="w-10 h-10 bg-zinc-900 border border-white/10 rounded-xl flex items-center justify-center text-gold-500 shadow-xl">
...
function MetricCard({ label, value, icon: Icon, growth, isAyrshare }: MetricCardProps) {
    return (
        <div className={`p-8 bg-zinc-900/40 border border-white/5 rounded-[2.5rem] hover:border-gold-500/20 transition-all group ${isAyrshare ? 'border-gold-500/5' : ''}`}>
            <div className="flex justify-between items-start mb-6">
                <div className={`w-10 h-10 bg-zinc-950 border border-white/5 rounded-xl flex items-center justify-center ${isAyrshare ? 'group-hover:text-signature-gradient' : ''}`}>
                    <Icon className="w-5 h-5 text-white/40 group-hover:text-gold-500 transition-colors" />
                </div>
                {growth !== undefined && (
            ...
            <p className="text-[10px] text-white font-black uppercase tracking-widest mb-1">{label}</p>
...
function SocialMetricCard() {
    return (
        <div className="p-8 bg-zinc-900/40 border border-white/5 rounded-[2.5rem] hover:border-gold-500/20 transition-all group">
            <div className="flex justify-between items-start mb-6">
