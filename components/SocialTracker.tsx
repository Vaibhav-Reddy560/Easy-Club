...
export default function SocialTracker({ clubs, onUpdateClub }: SocialTrackerProps) {
        ...
        >
            <header className="border-b border-white/5 pb-8 flex flex-col md:flex-row md:items-end justify-between gap-6">
                ...
                <div className="flex items-center gap-4">
                    <button 
                        ...
                        className="px-5 py-3 bg-[#050505] border border-white/10 rounded-2xl flex items-center gap-2 hover:bg-black transition-all group h-[50px]"
                    >
                        {loadingAyrshare ? <Loader2 className="w-4 h-4 text-gold-500 animate-spin" /> : <Clock className="w-4 h-4 text-gold-500" />}
                        <span className="text-[11px] text-gold-500 font-black uppercase tracking-widest">Sync</span>
                    </button>
            ...
            <AnimatePresence mode="wait">
                {activeClub && stats ? (
                    ...
                    >
                        {/* Ayrshare Configuration & Club Performance Section */}
                        {!activeClub.ayrshareApiKey || showConfig ? (
                            <div className="bg-[#050505] border border-white/15 rounded-[2.5rem] p-10 space-y-8">
                                <div>
                                    <h3 className="text-xl font-bold text-white mb-2 flex items-center gap-2"><Settings className="w-5 h-5 text-gold-500" /> Ayrshare Configuration</h3>
                                    <p className="text-[10px] text-zinc-100 font-bold uppercase tracking-widest">Connect your Unified Social Analytics API</p>
                                </div>
                                <div className="space-y-4">
                                    <label className="block text-[10px] font-black uppercase tracking-widest text-white mb-2">Ayrshare API Key</label>
                                    <input
                                        type="password"
                                        value={apiKeyInput || activeClub.ayrshareApiKey || ""}
                                        ...
                                        placeholder="Enter Ayrshare Primary API Key"
                                    />
                                    <p className="text-xs text-zinc-500">
                                        Generate an API key from your Ayrshare Dashboard (API Profile) to enable unified social analytics for {activeClub.name}.
                                    </p>
                                </div>
                                <div className="flex gap-4">
                                    <button
                                        onClick={() => {
                                            onUpdateClub({ ...activeClub, ayrshareApiKey: apiKeyInput });
                                            setShowConfig(false);
                                        }}
                                        className="px-6 py-3 bg-gold-gradient text-black font-black uppercase tracking-widest text-[10px] rounded-xl hover:scale-105 transition-all flex items-center gap-2"
                                    >
                                        <Save className="w-4 h-4" /> Save Configuration
                                    </button>
                                    {activeClub.ayrshareApiKey && (
                                        <button
                            ...
                            <div className="bg-[#050505] border border-white/15 rounded-[2.5rem] p-12 text-center space-y-8 relative overflow-hidden group shadow-2xl">
                                ...
                                <div className="relative z-10 space-y-6">
                                    ...
                                    <div className="flex flex-wrap justify-center gap-4">
                                        ...
                                        <button
                                            onClick={() => {
                                                setApiKeyInput(activeClub.ayrshareApiKey || "");
                                                setShowConfig(true);
                                            }}
                                            className="px-8 py-4 bg-white/5 border border-white/10 text-white font-black text-[10px] uppercase tracking-widest rounded-xl hover:bg-white/10 transition-all flex items-center gap-2"
                                        >
