      ...
      <div className="flex gap-2 p-1.5 bg-black/60 rounded-2xl border border-white/5 overflow-x-auto scrollbar-none">
        {FORMATS.map(f => (
          <button
            ...
            className={`px-5 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center gap-2 whitespace-nowrap transition-all ${activeTab === f.id ? "bg-gold-500 text-black shadow-lg shadow-gold-500/10" : "text-neutral-500 hover:text-white"}`}
          >
        ...
        {activeTab === "certificate" && (
          <motion.div key="cert" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="bg-neutral-900/40 border border-white/5 rounded-[3rem] p-10 text-center space-y-8">
            ...
            <div className="space-y-2">
              ...
              <p className="text-neutral-500 text-sm max-w-md mx-auto">For professional certificate designs, we recommend using Canva&apos;s template library. We&apos;ve selected the best category for your event type.</p>
            </div>
            <a
            ...
            <div className="grid grid-cols-1 xl:grid-cols-5 gap-8">
              ...
              <div className="xl:col-span-2 space-y-8">
                    ...
                    <div className="flex gap-2">
                      {POSTER_DIMS.map(d => (
                        <button
                          ...
                          className={`flex-1 px-3 py-2 rounded-xl text-[9px] font-bold uppercase transition-all border ${posterDim.label === d.label ? "bg-gold-500 text-black border-gold-500" : "bg-black/40 text-neutral-500 border-white/5 hover:border-white/10"}`}
                        >
                          {d.label}
                        </button>
                      ))}
                ...
                <section className="bg-neutral-900/40 border border-white/5 rounded-[2rem] p-6 space-y-4">
                  ...
                  />
                  <div className="flex justify-between text-[8px] text-neutral-600 uppercase tracking-widest font-bold">
                ...
                <section className="bg-neutral-900/40 border border-white/5 rounded-[2rem] p-6 space-y-4">
                  ...
                  {referenceImage ? (
                    ...
                    <label className="flex flex-col items-center gap-3 py-8 border-2 border-dashed border-white/10 rounded-2xl cursor-pointer hover:border-gold-500/30 transition-all group">
                      <Upload className="w-6 h-6 text-neutral-600 group-hover:text-gold-500 transition-colors" />
                      <span className="text-[9px] text-neutral-500 font-bold uppercase tracking-widest">Drop or click to upload</span>
                ...
                {!vibeAccepted && !useOverride && (
                  <p className="text-[9px] text-neutral-600 text-center uppercase tracking-widest">Suggest and accept a vibe first, or override with your own</p>
                )}
              ...
              <div className="xl:col-span-3 space-y-6">
                <div className="flex justify-between items-center">
                  <label className="text-[10px] text-neutral-500 font-bold uppercase tracking-widest">Live Preview — {currentFormat.label} {activeTab === "poster" ? `(${posterDim.dim})` : `(${currentFormat.dim})`}</label>
