        ...
        >
          ...
          <div className="mb-6 flex items-start justify-between pr-8">
            <div>
              ...
              <p className="text-zinc-300 text-[10px] uppercase font-bold tracking-widest mt-1">{eventName}</p>
              <p className="text-zinc-200 text-[9px] mt-1">
              ...
              <div className="bg-black/40 border border-white/5 rounded-2xl p-8">
                <div className="prose prose-invert prose-sm max-w-none">
                  {report.content.split('\n').map((line, i) => {
                    ...
                    if (line.startsWith('- ')) return <li key={i} className="text-sm text-zinc-300 ml-4 my-1 list-disc">{line.replace('- ', '')}</li>;
                    if (line.trim() === '') return <br key={i} />;
                    return <p key={i} className="text-sm text-zinc-300 leading-relaxed my-1">{line}</p>;
                  })}
                </div>
              </div>
            )}
